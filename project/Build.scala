...
object EmsDepends {
  ...
  val appDepends = Seq(beangle_config, typesafe_config, slf4j, logback_classic) ++
    ...
    Seq(beangle_jdbc, spring_tx, spring_aop, HikariCP, postgresql) ++
    Seq(beangle_cache, caffeine_jcache, jedis) ++
    Seq(beangle_model, hibernate_core, hibernate_jcache) ++
    Seq(beangle_security, beangle_template, beangle_serializer, beangle_event) ++
    Seq(scalatest)

  val webAppDepends = appDepends ++ Seq(beangle_she, freemarker)
}
