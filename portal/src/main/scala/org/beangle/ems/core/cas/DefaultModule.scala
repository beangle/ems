  ...
  override def binding(): Unit = {
    ...
    if (remoteCasServer.isDefined) {
      bind(classOf[CasConfig]).constructor($("remote.cas.server"))
        ...
        .property("localLoginUri", localLogin)
    ...
    //Cas使用
    val interceptor = bind("web.Interceptor.cas", classOf[SecurityInterceptor])
    ...
    // /cas/safety 是登录页链接的隐私安全政策页，必须匿名可达
    bind("security.Filter.authorization_cas", new AuthorizationFilter(
      new ProtectedAuthorizer(Set(localLogin, smsLogin, authLogin, "/cas/qrcode", "/cas/oauth", "/cas/safety"))))
    ...
    bind("web.Interceptor.security", classOf[SecurityInterceptor])
      .property("filters", list(ref("security.Filter.authorization")))
