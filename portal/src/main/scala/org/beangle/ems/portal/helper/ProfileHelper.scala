...
class ProfileHelper(entityDao: EntityDao, profileService: ProfileService, dimensionService: DimensionService) {
   ...
   * 查看限制资源界面
   */
  def populateInfo(profiles: Seq[EnvProfile], dimensions: Iterable[Dimension]): Unit = {
    val fieldMaps = new collection.mutable.HashMap[String, Map[String, AnyRef]]
    val dimensionMap = dimensions.map { x => (x.name, x) }.toMap
    for (profile <- profiles) {
      val aoDimensions = new collection.mutable.HashMap[String, AnyRef]
      for ((fieldName, v) <- profile.properties) {
        val value = v.toString
        val field = dimensionMap(fieldName)
    ...
    ActionContext.current.attribute("dimensionMap", dimensionMap)
    ActionContext.current.attribute("fieldMaps", fieldMaps)
  }

  def fillEditInfo(profile: EnvProfile, isAdmin: Boolean): Unit = {
    ...
    val myProfiles = entityDao.findBy(classOf[EnvProfile], "user.code", List(Securities.user))
    ...
    ActionContext.current.attribute("envs", dimensionService.getEnvs())
  }

  private def getMyProfileValues(profiles: Seq[EnvProfile], field: Dimension): collection.Seq[AnyRef] = {
  ...
  private def getIgnoreDimensions(profiles: Seq[EnvProfile]): collection.mutable.Set[String] = {
    val ignores = new collection.mutable.HashSet[String]
    for (profile <- profiles) {
  ...
  private def getProperty(profile: EnvProfile, field: Dimension): AnyRef = {
  ...
  def populateSaveInfo(profile: EnvProfile, isAdmin: Boolean): Unit = {
    ...
    for (field <- dimensionService.getAll()) {
      ...
      if ((ignoreDimensions.contains(field.name) || isAdmin) && Params.getBoolean("ignoreDimension" + field.id).getOrElse(false)) {
