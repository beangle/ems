...
<div id="lspace">
  <div id="portrait">
    ...
    <div id="olnks">
      <em>${user.fullname}</em>
      <ul>
        <li>[@b.a href="/admin/security/password!edit?user.id=${user.id}" target="user-info"]修改密码[/@]</li>
        [@ems.guard res="/admin/avatar/board"]<li>[@b.a href="/avatar/board!info?user.name=${user.name}"]更换头像[/@]</li>[/@]
      </ul>
    </div>
  </div>
