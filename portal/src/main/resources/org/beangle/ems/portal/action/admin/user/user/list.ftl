...
[@b.grid items=users var="user"]
  ...
  bar.addItem("${b.text("action.export")}",action.exportData("code:账户,name:姓名,category.name:身份,mobile:手机,email:电子邮箱,beginOn:创建日期,endOn:结束日期,roleNames:角色,enabled:是否启用,locked:是否锁定","Xlsx","&fileName=用户信息"));
 [/@]
  [@b.row]
    [@b.boxcol/]
    ...
    [@b.col property="group.name" title="默认组" width="8%"/]
    [@b.col title="角色"]
      [#assign members=[]]
